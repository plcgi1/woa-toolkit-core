...
__PACKAGE__->mk_accessors(qw/args_filled req_method_matched ok_status_map error_message_map env logger cookies headers/);
...
sub cleanup {
    my($self)=@_;
    foreach ( qw/content_type output status location error_message current_method args_filled/ ){
        delete $self->{$_};
    }
    return;
}
...
sub process {
    ...
    unless ( $self->req_method_matched ) {
        ...
        return $self->set_error(
            400,
            "[BAD REQUEST] Request Method for '".$self->request->method."' not implemented or not defined in map",
            $method_data
        );
    }
    ...
    if( ref $is_valid eq 'WOA::Validator::ErrorCode' ){
        ...
        if ( $error_message_map->{400} eq $ERROR_MESSAGE_MAP->{400} ){
            $msg = "[BAD REQUEST] Method: $method. Input params not valid.Data: ".Dumper $args;
        }
        else {
    ...
    unless ( $self->backend ){
        return $self->set_error(500,"[BACKEND NOT DEFINED] You must define backend object",$method_data);
    }
    ...
    # if_errors
    unless ( $res ){
        return $self->set_error(404,"[NOT FOUND] Method: $method. Params:".Dumper $adopted_args,$method_data);
    }
...
sub get_from_map {
    ...
    foreach ( @{$self->map} ){
        ...
        if( $_->{regexp} ){
            ...
            if($path=~/$re/){
                $url_matched = 1;
                unless ( ($_->{in}) && (ref $_->{in} eq 'HASH') && ($_->{in}->{param}) ) {
                    $qs_matched = 1;
                }
            }
            if( $_->{in} && $url_matched ){ 
...
sub args_for_method {
    ...
    my $rule = ( $method_data->{in} ->{param} && ref $method_data->{in}->{param} eq 'ARRAY' ); 
    if( !$self->args_filled() ){
        if ( $rule ){
